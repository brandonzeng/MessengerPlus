...
def get_individuals(result, individuals):
    dates = {}
...
friend = get_individuals(result, [i[0] for i in top_X_reconnects])
data = Data([
    Scatter(
        x = sorted(dates.keys()),
    ...
    Scatter(
        x = sorted(friend.keys()),
        y = [friend[i] for i in sorted(friend.keys())],
...
layout = Layout(
    ...
    yaxis = YAxis(title = "Messages exchanged")
)
fig = Figure(data = data, layout=layout)
plot_url = py.plot(fig, filename = "Messenger Plus", fileopt = "overwrite")
print plot_url     
